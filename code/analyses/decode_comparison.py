...
parser.add_argument('--regions', default=None, nargs='*', type=str, help='Probe name to plot (will ignore args.channel-name/num), e.g., LSTG')
...
parser.add_argument('--data-type_filters', choices=['micro_high-gamma','macro_high-gamma', 'micro_raw','macro_raw', 'spike_raw'], nargs='*', default=[], help='Only if args.regions is used')
...
if args.regions:
    ...
    args.patient, args.data_type, args.filter, args.probe_name = get_patient_probes_of_region(args.regions, args.data_type_filters)
print(args)
...
if len(comparison['queries']) > 2:
    ...
    if args.gat:
        ...
        time_gen = SlidingEstimator(clf, n_jobs=-1, scoring='roc_auc', verbose=True)
