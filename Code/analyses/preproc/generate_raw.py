...
parser.add_argument('--filter', choices = ['raw', 'gaussian-kernel', 'gaussian-kernel-25', 'high-gamma'], default='raw', help='raw/gaussian-kernel.')
...
if args.data_type != 'spike':
    if args.filter in ['raw', 'gaussian_kernel']:
        ################
        # NOTCH (line) #
        ################
        if args.data_type in ['raw', 'gaussian-kernel']: # High-gamma is filtered and extracted by aMatlab script, then if args.data_type != 'spike'saved into fif
            raw.notch_filter(params.line_frequency, fir_design='firwin') # notch filter
        ...
        data_scaled = np.transpose(transformer.transform(np.transpose(data))) # num_channels X num_timepoints
        ...
        raw._data = np.transpose(transformer.inverse_transform(np.transpose(data_scaled)))
...
# SMOOTH #
##########
if args.filter == 'gaussian-kernel': # smooth raw data
    # get data and smooth
    ...
    raw._data = data
elif args.filter == 'gaussian-kernel-25': # smooth raw data
    # get data and smooth
    width_sec = 0.025 # Gaussian-kernal width in [sec]
    data = raw.copy().get_data()
    for ch in range(data.shape[0]): # Loop over channels
        time_serie = data[ch, :]
        data[ch, :] = gaussian_filter1d(time_serie, width_sec*raw.info['sfreq'])
