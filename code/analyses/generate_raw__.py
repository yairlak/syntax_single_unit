...
parser.add_argument('--overwrite',
                    default=False, action='store_true',
                    help='If false and fif file already exists then the script is stopped.')
...
exists = os.path.isfile(os.path.join(path2rawdata, 'mne', filename))
if exists and not args.overwrite:
    print(f'WARNING: fif file {filename} already exists - nothing was done.')
    exit()
...
print('Raw fif saved to: %s' % os.path.join(path2rawdata, 'mne', filename))
