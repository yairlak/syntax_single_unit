...
parser.add_argument('--filter', help= 'raw/gaussian-kernel-??/high-gamma',
...
parser.add_argument('--min-n-trials', default=1,
                    help='Minimal number of trials per class (e.g., word)')
# MODEL
...
parser.add_argument('--tmax_window', default=0.6, type=float,
...
X = []
IX_frequent_words = np.ones_like(words, dtype=bool)
words = np.asarray(words)
for i_word, word in enumerate(words):
    x_i = epochs_neural_sentence[f'word_string=="{word}"'].\
                        get_data()
    if x_i.shape[0] < args.min_n_trials:
        IX_frequent_words[i_word] = False
        continue
    X.append(x_i.mean(axis=0))
X = np.asarray(X)
print(words, IX_frequent_words)
words = words[IX_frequent_words]
...
for ch, ch_name in enumerate(epochs_neural_sentence.ch_names):
    ...
    # SAVE #
    ########
    with open(os.path.join(args.path2output, fname_fig[:-4] + '.clu'), 'wb') as f:
        pickle.dump([DSM, clustering, dendro, words, ch_name, args], f)
