...
# DATA
parser.add_argument('--patient', action='append', default=['479_11', '502'],
                    help='Patient number')
parser.add_argument('--data-type', choices=['micro','macro', 'spike'],
                    action='append', default=['spike', 'spike'])
parser.add_argument('--level',
                    choices=['sentence_onset','sentence_offset',
                             'word', 'phone'],
                    default=None)
parser.add_argument('--filter', choices=['raw', 'high-gamma'],
                    action='append', default=['raw', 'raw'], help='')
...
parser.add_argument('--channel-name', default=['p_g1_44_GB2-RPS', 'GB4-RFSG1_57p1', 'GB4-RFSG4_60p1'], nargs='*', action='append',
...
# QUERY
parser.add_argument('--comparison-name', default='first_word',
...
for i_class, curr_class in enumerate(classes):
    ...
    for i_split, (IXs_train, IX_test) in enumerate(loo.split(X, y)):
        ...
        if (args.GAC or args.GAM) and i_split == 0: # Use all training data once
            ...
            if args.multiclass:
                y_multi = np.zeros_like(y)
                y_multi[y==curr_class] = 1
                temp_estimator.fit(X[IXs_train], y_multi[IXs_train])
            else:
        ...
        if (args.GAC or args.GAM): # Use all training data once
            ...
            y_hats.append(proba[0, :, -1])
            if args.multiclass:
                y_trues.append(y_multi[IX_test])
            else:
...
# STATS
scores_multi = np.asarray(scores_multi)
...
fname_pkl = os.path.join(args.path2output, 'multi_' + fname_pkl)
with open(fname_pkl, 'wb') as f:
     pickle.dump([scores_multi, pvals, effect_sizes, data.epochs[0].times, temp_estimator, clf, comparisons, (stimuli, stimuli_gen), args], f)
