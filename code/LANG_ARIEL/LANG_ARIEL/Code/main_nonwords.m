function main_nonwords(subject,run)
% Add_Psych;
%% Initialize
warning off
% HideCursor
...
for i = 1:params.num_nonwords
    stimuli(i).image = imread(fullfile(settings.path2nonwords, sprintf('%i.png', i)));
...
%% Begin experiment
middleKey = KbName('m');
subject_responses = cell(params.num_nonwords, 1);
...
DisableKeysForKbCheck([]);
Screen('CloseAll');
filename = sprintf('Subject_%s_run_%i_nonwords', subject, run);
