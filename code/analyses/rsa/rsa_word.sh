    ...
    for min_trials in $MIN_TRIALs; do
        for time_window in $TIME_WINDOWs; do
            for num_bins in $NUM_BINs; do
                for T in $Ts; do
                    CMD='python /neurospin/unicog/protocols/intracranial/syntax_single_unit/code/analyses/rsa.py --level word --patient 493 --data-type micro --filter raw --probe-name RFSG --patient 502 --data-type micro --filter raw --probe-name RFSG LFSG --patient 505 --data-type micro --filter raw --probe-name LFGA LFGP --patient 515 --data-type micro --filter raw --probe-name LFSG RFSG --patient 493 --data-type spike --filter raw --probe-name RFSG --patient 502 --data-type spike --filter raw --probe-name RFSG LFSG --patient 505 --data-type spike --filter raw --probe-name LFGA LFGP --patient 515 --data-type spike --filter raw --probe-name LFSG --patient 502 --data-type macro --filter raw --probe-name RFSG LFSG --patient 505 --data-type macro --filter raw --probe-name LFGP --patient 515 --data-type macro --filter raw --probe-name LFSG RFSG --comparison-name word_string --block-type auditory --min-trials '$min_trials' --time-window '$time_window' --num-bins '$num_bins' --times '$T' --model-type '$model_type
