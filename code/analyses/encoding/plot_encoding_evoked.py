...
for i_channel, ch_name in enumerate(ch_names):
    ...
    print(results['full']['scores_by_time'][0].shape)
    times = np.arange(results['full']['scores_by_time'][0].shape[0])
    print(times)
    fig_r2 = plot_rf_r(times, results, i_channel, ch_name, feature_info, args)
    fname_fig = os.path.join(args.path2figures, 
                             args.patient[0],
                             args.data_type[0],
                             'rf_r_' +
